...
export function OverviewKPIs({ overall }: { overall: PipelineAggregate | null }) {
  ...
  const avgAI = overall?.avgAI ?? null;
  const avgLatency = overall ? overall.avgLatency : 0;
  const successRate = overall && totalTests
    ...
    <Grid container spacing={2} sx={{ mb: 2 }}>
      <Grid size={{ xs: 12, md: 3 }}>
        <Card><CardContent>
          ...
          <Typography variant="h4">{successRate === null ? 'N/A' : `${successRate.toFixed(1)}%`}</Typography>
      ...
      <Grid size={{ xs: 12, md: 3 }}>
        <Card><CardContent>
          ...
          <Typography variant="h4">{avgAI === null ? 'N/A' : `${avgAI.toFixed(1)}%`}</Typography>
