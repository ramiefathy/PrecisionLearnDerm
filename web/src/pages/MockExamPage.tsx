...
type Loader = (topicIds: string[], count: number) => Promise<{ itemId: string }[]>;
type ItemLoader = (itemId: string) => Promise<any>;

export default function MockExamPage({ loadBatch, loadItem }: { loadBatch?: Loader; loadItem?: ItemLoader } = {}){
  ...
  useEffect(()=>{ if(running && secondsLeft===0){ submit(); } },[secondsLeft,running]);
  ...
  async function start(){
    ...
    while(remaining > 0){
      ...
      let previews: { itemId: string }[] = [];
      if (loadBatch) {
        previews = await loadBatch(Array.from(new Set(sampleTopics)), batchSize);
      } else {
        const res:any = await (api as any).pe.nextItems({ topicIds: Array.from(new Set(sampleTopics)), count: batchSize });
        previews = Array.isArray(res?.items) ? res.items : [];
      }
      for(const p of previews){
        if(p?.itemId){ const full = loadItem ? await loadItem(p.itemId) : await api.items.get(p.itemId); items.push({ itemId: p.itemId, fullItem: full }); }
      }
      remaining -= batchSize;
    }
  ...
  function choose(i:number, opt:number){ setAnswers(prev=>({ ...prev, [i]: opt })); }
