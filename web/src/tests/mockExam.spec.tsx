...
const mockItem = { stem: "Mock Q", leadIn: "Which?", options: [{text:"A"},{text:"B"},{text:"C"},{text:"D"}], keyIndex: 0, topicIds: ["psoriasis.plaque"] };
...
describe("MockExamPage", () => {
  it("submits attempt with injected loaders and correct payload shape", async () => {
    ...
    const calls = (saveAttempt as unknown as ReturnType<typeof vi.fn>).mock.calls;
    ...
    expect(payload.items.some((it: any) => it.itemRef === 'm1' && it.chosenIndex === 0 && it.correct === true)).toBe(true);
  });
});
