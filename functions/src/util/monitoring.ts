...
export async function log(entry: Partial<LogEntry>): Promise<void> {
  ...
  try {
    ...
    // Maintain legacy + new fields during migration
    logDocument.timestamp = fullEntry.timestamp;
    try {
...
/**
 * Updated getLogs to support dual-field (createdAt preferred, timestamp fallback)
 */
...
/**
 * Helper function to record MCQ generation metrics
 */
